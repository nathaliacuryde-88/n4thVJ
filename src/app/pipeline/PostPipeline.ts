...
export class PostPipeline {
  ...
  /** True when at least one effect is doing something. */
  ...
  /**
   * Push one frame of `source` through the chain and present it.
   * `time` is seconds; it only drives the displacement field.
   */
  render(source: HTMLCanvasElement, time: number) {
    const gl = this.gl;
    ...
    if (!this.active) {
      ...
      bindTexture(gl, program, 'uTex', this.sourceTexture, 0);
      drawFullscreen(gl, null, width, height);
      return;
    }

    const c = this.cfg;
    let current: WebGLTexture = this.sourceTexture;
    let target: RenderTarget;

    if (c.feedback.amount > 0) {
    ...
    if (c.quantize.pixel > 1 || c.quantize.levels >= 2) {
